...
const NAV_LINK_CLASSES =
  "rounded-lg px-3 py-1.5 text-zinc-600 hover:bg-zinc-100 dark:text-zinc-300 dark:hover:bg-zinc-800";
...
export function HeaderActions() {
  ...
  return (
    <div className="flex items-center gap-3">
      {auth.authenticated && auth.role === "PASSENGER" && (
        <div className="flex items-center gap-1 text-sm">
          <Link href="/rides" className={NAV_LINK_CLASSES}>
            Rides
          </Link>
          <Link href="/rides/new" className={NAV_LINK_CLASSES}>
            Request a ride
          </Link>
        </div>
      )}
      {auth.authenticated && auth.role === "DRIVER" && (
        <div className="flex items-center gap-1 text-sm">
          <Link href="/driver/dashboard" className={NAV_LINK_CLASSES}>
            Driver dashboard
          </Link>
        </div>
      )}
