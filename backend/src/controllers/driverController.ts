...
export const cancelRideDriverHandler = async (req: Request, res: Response): Promise<void> => {
  const ride = await cancelRideForDriver(
    req.user!.id,
    Number(req.params.rideRequestId),
    (req.body as CancelRideDriverInput).reason,
  );
  res.json({ ride });
};
